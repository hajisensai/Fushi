...
/// BUG-845：有声书播放中按返回（手势侧滑 / 返回键 / ESC）无反应，等音频真停下来
/// 那一刻才连着止声一起退出书。
///
...
/// 读的 `isActive` / `book`）由 `stop()` 的**同步首段**归零，Dart 里 async 函数在
/// 首个 await 之前同步执行，所以 fire-and-forget 同样满足 TODO-831 的「pop 动画
/// 首帧不闪播放条」；native 资源释放与「用户已经离开这一页」没有因果关系。
///
/// 本文件是**行为层**证据：native 释放挂起时，`stop()` 返回的 Future **不完成**
/// （证明 await 它就会把用户的返回吞掉），但同步首段已把会话清空并通知监听者
...
void main() {
  ...
  group('AudiobookSession.stop 的同步首段契约 (BUG-845)', () {
    test('native 释放挂起时 stop() 不完成，但会话已同步清空并通知', () async {
      ...
      // 停止时才有解码器要释放——这正是 BUG-845 只在播放态出现、暂停态没事的原因。
      ...
      // 同步首段契约：调用返回的这一刻（还没跑任何微任务）会话就已经空了，
      // 书架 NowListeningMiniBar 在 pop 动画首帧即见空会话（TODO-831 不闪播放条）。
      expect(session.isActive, isFalse,
          reason: 'stop() 的同步首段必须清空会话（不能等 await 段完成）');
      expect(session.controller, isNull);
      expect(session.book, isNull);
      expect(notifications, greaterThanOrEqualTo(1),
          reason: 'stop() 的同步首段必须 notifyListeners，让书架立刻收起播放条');

      // 而 stop() 本身仍挂在 native 释放上——旧实现 `await` 它，等于把用户的返回
      // 一起挂住（并被 _popInProgress 吞掉后续每一次返回），正是 BUG-845 的症状。
      await Future<void>.delayed(const Duration(milliseconds: 50));
