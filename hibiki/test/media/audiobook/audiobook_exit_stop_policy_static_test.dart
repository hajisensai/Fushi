...
void main() {
  ...
  //
  // BUG-845 契约变更：这里从「await stop」改成「unawaited(stop())」。TODO-831 要的
  ...
  test('onSourcePagePop fires the guarded stop without awaiting (BUG-845)', () {
    ...
    // 负向：绝不能退回 await——那正是 BUG-845 的根因（播放中返回被吞到音频停下）。
    expect(
      RegExp(r'await\s+appModel\.audiobookSession\.stop\(\)').hasMatch(pop),
      isFalse,
      reason: 'BUG-845：退出路径不得 await stop（native 释放耗时不可控且只在播放态'
          '发生，会把用户的返回挂到音频真停下来那一刻）',
    );
  });

  // W1（TODO-831 复核 → BUG-845 收口）：stop 在释放 native 解码器时可能抛平台异常。
