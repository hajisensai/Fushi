...
class _AudioTracksCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool explainedEmpty =
        state.audioTracks.isEmpty && emptyHint != GalTrackEmptyHint.generic;
    return _SectionCard(
      ...
      child: explainedEmpty
          ? _DetailBox(
              icon: Icons.info_outline,
              text: emptyHint == GalTrackEmptyHint.resourceMode
                  ? t.game_tracks_resource_mode_hint
                  : t.game_tracks_loopback_hint,
            )
          : Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: <Widget>[
