...
/// 字幕列表行的**固定几何**（BUG-1025）。行高由 `itemExtentBuilder` 事先给出，若与真实
...
/// 行内字幕**文本列可用宽度**（BUG-1025）。纯函数，行高测量与真实渲染同源。
...
class _VideoSubtitleJumpPanelState extends State<VideoSubtitleJumpPanel> {
  ...
  /// 行高测量缓存（BUG-1025），key = `加粗位 + 文本`。宽度 / 字号 / textScaler 变化时整体
  ...
  /// 保证 `itemExtentBuilder` 给出的行高与真实换行结果一致（BUG-1025）。
  ...
  /// （与 [_buildRowText] 的 `selected || selectedForCard` 同源，BUG-1025）。
  ...
  Widget build(BuildContext context) {
    ...
    _rowHitCues.clear();
    // BUG-1025：行高测量必须用与渲染一致的排版环境（文字缩放 / 书写方向）。
              ...
              child: Listener(
                ...
                child: showLoading
                    ...
                    : cues.isEmpty || visibleIndexes.isEmpty
                        ...
                        // 行高按真实文本布局测量（[_rowExtentForCue]，BUG-1025），与
    ...
    return MouseRegion(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          //
          // BUG-1025：色条占的 3px 从**左内缩里扣**（5 + 3 = 8），使内容起点与文本列宽
  ...
  Widget _buildRowText(
    ...
    // BUG-1025：与行高测量（[_measureRowExtent]）共用同一样式，断行结果一致，末行不被裁。
  ...
  Widget _buildSelectionCheckbox(
    ...
    // BUG-1025：勾选框列宽 / 高锁死为 [kSubtitleRowSelectionSize]（即 compact + shrinkWrap
