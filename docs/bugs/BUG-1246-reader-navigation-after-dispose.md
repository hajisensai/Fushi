...
- **[x] ① 已修复** — `AudiobookSession` 在每次 reader attach/detach 时递增 owner
  ...
  cancel/finally 均不能触碰新 owner。`_navigateToChapter` 和 `_rebuild` 的同步入口也通过
  可直接测试的生产门控，在任何副作用前拒绝已销毁 reader。原业务修复基线：
...
- **[x] ② 已加自动化测试** —
  ...
  定向及相邻回归共 67 项通过；故障注入确认无效/后移 guard、漏释放、漏 await 后复检、
  旧 owner 仅凭 controller identity 穿透均会使测试失败。
