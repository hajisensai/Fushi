...
- **[x] ① 已修复** — 把布尔闸改成带截止时间的在途判定：`hibikiPendingSince` 记发起时刻，`if (hibikiPending && Date.now() - hibikiPendingSince < HIBIKI_PENDING_TIMEOUT_MS) return;`。回调仍正常复位；截止时间是「回调永不触发」的安全兜底，超时后放行新查词，死锁彻底消失。两份镜像同步。提交：e413d2d58
- **[x] ② 已加自动化测试** — `tools/browser-extension/shift-hover.test.js` 新增用例：stub `sendMessage` **不回调**（模拟 worker 被杀），推进 `Date.now` 越过超时后再发 Shift mousemove，断言仍能发出第二次 `{type:'lookup'}`（旧代码此处永久卡死）。提交：e413d2d58
