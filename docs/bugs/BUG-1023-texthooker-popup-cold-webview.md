## BUG-1023 · 捕获工作台查词弹窗每次冷建WebView加载缓慢
...
- **[ ] ① 未修复** — 方案：`TexthookerPage.initState` 对齐 `home_dictionary_page` 做 `seedWarmSlot()` 预热（含 lowMemory 赋值），`_onWordTap` 改 `reuseWarmSlot:true`，冷建成本从每次点词提前到进页一次。
- **[ ] ② 未加自动化测试** — 方案：源码扫描守卫（texthooker_page.dart 必须含 seedWarmSlot 与 reuseWarmSlot）。
- **备注**：与 BUG-1022 同轮 galgame Hook UX 重构（工具栏重组/音轨自动刷新/游戏库卡片精简）一并落地。
