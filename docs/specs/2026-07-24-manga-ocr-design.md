...
- 状态：**已拍板，待实现**（P0 四项决策已由用户 2026-07-24 确认，见 §8）
...
## 8. P0 决策记录（用户已于 2026-07-24 拍板）

1. **P2 主路径**：✅ app 内置 ONNX 整卷 OCR 为主，外部 mokuro CLI 降为可选后备。
2. **移动端整卷 OCR**：✅ P4 真机基准后再定去留（先做桌面 + 互联；单框补扫不受影响）。
3. **云端 VLM 手写兜底**：✅ 做，进 P4（逐框、手动触发、默认关、自备 API key、明示该框图片上云）。
4. **数据模型与格式范围**：✅ `EpubBooks.format='manga'`（schema v52）+ 只支持 mokuro v0.2+ 新格式（不支持旧版单 HTML）。
