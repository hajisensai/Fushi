...
class AnkiConnectRepository extends BaseAnkiRepository {
  ...
  Future<RenderedMinedFields> _renderMinedFields({
    ...
    // Sentence audio is part of the requested Galgame card, not an optional
    // decoration. Upload it before duplicate probes, word audio, cover art and
    // dictionary media can compete for AnkiConnect. If it fails, abort before
    // addNote/updateNoteFields instead of silently writing an empty field.
    final String? sasayakiMediaRef = context.sasayakiAudioPath != null
        ? await _storeRequiredLocalMedia(
            service,
        ...
        : null;
    if (context.requireCover &&
        (context.coverPath == null || context.coverPath!.isEmpty)) {
      throw const FileSystemException('Required card picture is missing');
    }
    final String? requiredCoverMediaRef = context.requireCover
        ? await _storeRequiredLocalMedia(
            service,
            context.coverPath!,
            'hibiki_cover_',
    ...
    final List<Future<dynamic>> mediaFutures = <Future<dynamic>>[
      context.coverPath != null && !context.requireCover
    ...
    final String? coverMediaRef =
        requiredCoverMediaRef ?? mediaResults[0] as String?;
    final AudioFetchOutcome remoteAudio = mediaResults[1] as AudioFetchOutcome;
    final Map<String, String> dictionaryMediaTags =
        mediaResults[2] as Map<String, String>;
  ...
  Future<String> _storeRequiredLocalMedia(
    AnkiConnectService service,
    String filePath,
    String prefix, {
    ...
    try {
      ...
      if (!file.existsSync()) {
        throw FileSystemException(
            'Required $description does not exist', filePath);
      }
      final bytes = await file.readAsBytes();
      final filename = hibikiAnkiMediaFilenameForBytes(
        prefix: prefix,
      ...
      debugPrint(
        'AnkiConnectRepository._storeRequiredLocalMedia: $e\n$stack',
      );
      rethrow;
    }
  }
